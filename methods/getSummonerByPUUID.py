...
def get_summoner_by_puuid(puuid:str,region:str, apiInstance:riot_api, lang:str = "en_US"):
    ...
    try:
       riot_url = requests.get(f'https://{region_routing_value}.api.riotgames.com/riot/account/v1/accounts/by-puuid/{puuid}?api_key={api_key}').json()

       if 'status' in riot_url:
           handle_api_error(riot_url['status']['status_code'])

       summoner_url = requests.get(f'https://la2.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/{puuid}?api_key={api_key}').json()
